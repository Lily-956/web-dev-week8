import './home.css'

function Home() {
    return (
        <div>
            <h2>
            Welcome to my website! I hope you enjoy your stay  ~(˘▾˘~)
            </h2>
            <h3>
            (in case you would like some food and drinks during your stay: 🍙🥤)
            </h3>
            <img className="home" src="https://cdn.shopify.com/s/files/1/0747/3829/products/mHP1567_1024x1024.jpeg?v=1571444587" alt="Minion poster" />
            <h4>
            This website will contain an overview, character descrptions, movie summaries, and fun facts about the movie
            </h4>
        </div>
    )
}

export default Home